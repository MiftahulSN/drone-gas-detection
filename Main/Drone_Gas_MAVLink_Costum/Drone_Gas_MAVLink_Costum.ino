...
unsigned long stream_frequency = 100;
...
void costume_sampling_interval();
...
void stream_sampling_msg(Data_sampling sampling);
...
/*
  Stream MQ Gas Sensor with Costume Messages via MAVLink to Pixhawk
// */
void stream_sampling_msg(Data_sampling sampling) {
  mavlink_message_t msg;
  uint8_t buf[MAVLINK_MAX_PACKET_LEN];

  mavlink_msg_data_sampling_pack(
    ESP_SYS_ID,               // system ID        (1: Become one with pixhawk sys_ID)
    ESP_COMP_ID,              // component ID     (158: Generic autopilot peripheral comp_ID)
    &msg,                     // mavlink message
    sampling.sequence,        // mission sequence
    sampling.MQ2,             // mq2 gas value
    sampling.MQ5,             // mq5 gas value
    sampling.lat,             // lattitude GPS coordinate
    sampling.lon              // longitude GPS coordinate
  );
...
/*
  Interval Frequency of Gas Sampling Updated Stream
*/
void costume_sampling_interval() {
  sampling.sequence = 0;
  sampling.MQ2 = get_ppm(MQ2_pin, MQ2_rl, MQ2_ro, MQ2_m, MQ2_b);
  sampling.MQ5 = get_ppm(MQ5_pin, MQ5_rl, MQ5_ro, MQ5_m, MQ5_b);
  if (millis() - last_stream >= stream_frequency) {

    stream_sampling_msg(sampling);
    stream_sampling_msg(sampling);
